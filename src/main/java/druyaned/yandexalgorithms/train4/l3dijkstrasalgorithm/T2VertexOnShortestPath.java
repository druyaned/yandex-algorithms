...
public class T2VertexOnShortestPath {
    ...
    public static void solve(BufferedReader reader, BufferedWriter writer) throws IOException {
        ...
        int dist = destinations[finish].distance;
        showDestinations(n, destinations, reverseDestinations);
        showVerticesOnShortestPath(n, dist, destinations, reverseDestinations);
        showEdges(k, edges);
        showEdgesOnShortestPath(k, dist, destinations, reverseDestinations, edges);
    }
    ...
    private static void showVerticesOnShortestPath(int n, int distance,
            Destination[] destinations, Destination[] reverseDestinations) {
        final Destination[] d = destinations;
        final Destination[] rd = reverseDestinations;
        System.out.print("verticesOnShortestPath:");
        for (int i = 1; i <= n; i++) {
            if (d[i] == null) {
                System.out.print(" n");
            } else if (d[i].distance + rd[i].distance == distance) {
                System.out.print(" Y");
            } else {
                System.out.print(" n");
            }
    ...
    private static void showEdgesOnShortestPath(int k, int distance,
            Destination[] destinations, Destination[] reverseDestinations, Edge[] edges) {
        final Destination[] d = destinations;
        final Destination[] rd = reverseDestinations;
        final Edge[] e = edges;
        System.out.print("edgesOnShortestPath: ");
        for (int i = 1; i <= k; i++) {
            if (destinations[e[i].v1] == null) {
                System.out.print(" n");
            } else if (d[e[i].v1].distance + e[i].weight + rd[e[i].v2].distance == distance) {
                System.out.print(" Y");
            } else {
                System.out.print(" n");
            }
